...
class UnifiedMCPOrchestrator:
    ...
    def _build_langgraph_workflow(self):
        ...
        workflow.add_node("layer1_input_sanitization", self.workflow_nodes.layer1_input_sanitization)
        workflow.add_node("layer2_semantic_and_entity_analysis", self.workflow_nodes.layer2_semantic_and_entity_analysis)
        workflow.add_node("layer3_business_context", self.workflow_nodes.layer3_business_context)
        workflow.add_node("layer4_final_approval", self.workflow_nodes.layer4_final_approval)
        workflow.add_node("discover_models", self.workflow_nodes.discover_models)
        ...
        workflow.add_conditional_edges(
            ...
            {"approved": "layer1_input_sanitization", "blocked": END}
        )
        workflow.add_edge("layer1_input_sanitization", "layer2_semantic_and_entity_analysis")
        workflow.add_edge("layer2_semantic_and_entity_analysis", "layer3_business_context")
        workflow.add_edge("layer3_business_context", "layer4_final_approval")
        workflow.add_conditional_edges(
            "layer4_final_approval",
            self._route_after_security,
            {"approved": "discover_models", "blocked": END}
        )
        workflow.add_edge("discover_models", "execute_query")
    ...
    async def _generate_response(self, state: MCPAgentState) -> MCPAgentState:
        print("📝 Step 8: Response Generation")
